                ...
                <div class="collapse navbar-collapse" :class="{ 'show': navbarOpen }" id="navbarSupportedContent">
                    <ul class="navbar-nav mr-auto">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link" href="#" @click="toggleDropdown" id="navbarDropdown" role="button"
                                aria-haspopup="true" :aria-expanded="navbarOpen">
                                Sản phẩm
                            </a>
                            <div class="dropdown-menu" :class="{ 'show': dropdownOpen }" aria-labelledby="navbarDropdown">
                                <router-link v-for="cat in categories" :key="cat.id" :to="'/category?catId=' + cat.id"
                                    class="dropdown-item">Danh mục: {{ cat.name }}</router-link>
                            </div>
                        </li>
                    </ul>
                    <form @submit.prevent="searchBookHandler" class="mx-2 form-inline">
                        <input class="form-control" v-model="searchQuery" placeholder="Tìm kiếm..." type="text" name="q"
                            required>
                    ...
                    <div v-if="authStore.user" class="dropdown">
                        ...
                        <button type="button" class="btn dropdown-toggle" @click="toggleUserDropdown">
                            <img :src="authStore.user.image" style="height: auto; width: 50px;" alt=""
